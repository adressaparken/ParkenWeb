$(document).ready(function() {
    ...
    $.ajax({
        type: 'get',
        dataType: 'json',
        url: "https://adressaparken.hackheim.no/api/v1/sensorDataList?sensor=NO2&limit=100&offset=0", //this is the API url request to get the last 100 results for NO2 sensor
    ...
    $.ajax({
        type: 'get',
        dataType: 'json',
        url: "https://adressaparken.hackheim.no/api/v1/sensorDataList?sensor=LUM&limit=100&offset=0", //this is the API url request to get the last 100 results for LUM sensor
        success: function(result) {
    ...
    function CreateAreChart(data, id, name) {

        var margin = { top: 15, right: 13, bottom: 15, left: 23 },
        ...
        var xAxis = d3.svg.axis()
            .scale(x)
            .ticks(d3.time.hours, Math.floor(data.length / 25))
            .tickSize(-height)
            .tickPadding([6])
            .orient("bottom");
    ...
    //this function creates the chart
    function CreateLumAreChart(data, id, name) {
        var margin = { top: 15, right: 13, bottom: 15, left: 23 },
            width = $(id).width() - margin.left - margin.right,
            height = $(id).height() - margin.top - margin.bottom;
        ...
        var xAxis = d3.svg.axis()
            .scale(x)
            .ticks(d3.time.hours, Math.floor(data.length / 55))
            .tickSize(-height)
            .tickPadding([6])
            .orient("bottom");
